...
class HomeGrades(private val a: Ash) : Screen(a) {
    override fun show() {
        ...
        ash.headerPlain.text = ash.act.resources.getText(R.string.grades_label)
        ...
        ash.userData.subjects.forEachIndexed { index, s ->
            val grades = ash.userData.getGradesBySubject(index)
            ...
            if (grades.size > 0) {
                ...
                grades.forEach {
                    val gradeView = TextView(ash.act)
                    gradeView.text = "${ash.act.resources.getText(R.string.grade)}: " + it.value + "\n${ash.act.resources.getText(R.string.weight)}: " + it.weight + "\n${ash.act.resources.getText(R.string.description)}: " + it.description + "\n"
                    gradeView.textSize = 18.0f
                    scrollLayout.addView(gradeView)
                }
            }
        }
        ...
        if (scrollLayout.childCount == 0)
        {
            ...
            infoView.setText(ash.act.getString(R.string.no_grades))
            scrollLayout.addView(infoView)
        }

        val gradeAdd = TextView(ash.act)
        gradeAdd.gravity = Gravity.CENTER
        gradeAdd.layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT)
        gradeAdd.text = "+"
        gradeAdd.setTextSize(Dimension.SP, 20.0f)
        gradeAdd.setTextColor(-15540377)
        gradeAdd.setTypeface(ash.act.resources.getFont(R.font.fa_solid))
        gradeAdd.setPadding(paddingHorizontal, paddingVertical, paddingHorizontal, paddingVertical)

        gradeAdd.setOnClickListener {
//                showManualTimetableSelect(index)
//            ash.manualTimetableSelect.dayIndex = index
//            ash.showScreen(ash.manualTimetableSelect)
            ash.manualGrade.reset()
            ash.showScreen(ash.manualGrade)
        }

        scrollLayout.addView(gradeAdd)
        ...
        ash.homeButtonsGrades.children.iterator().forEach { textView ->
